...
class Baseline:
    ...
    def __init__(self, x_size, y_size, w_stddev, **kwargs):
        ...
        self.W = tf.Variable(tf.random.normal((self.x_size, self.y_size), stddev=self.w_stddev), name='W')
    ...
    def dot(self, X, Y, name='dot_op'):
        with tf.name_scope(name) as scope:
            dot_op = tf.linalg.tensor_diag_part(tf.matmul(X, Y, transpose_b=True))
            return dot_op
