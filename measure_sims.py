...
for hypo, hyper_vec in zip(test,hyper_vecs):
    if len(hyper_vec) == 1: ## this happens if your OOV_STRATEGY == 'top_hyper'; in this case return the sorted list of 10 most frequent hypernyms
        for line in top_ten:
...
with zipfile.ZipFile(OUT+archive_name, 'w') as file:
        file.write('%s%s_%s_%s_%s.tsv' % (OUT, VECTORS, POS, MODE, OOV_STRATEGY), '%s_%s_%s_%s.tsv' % (VECTORS, POS, MODE, OOV_STRATEGY))

end = time.time()
training_time = int(end - start)

print('\n%%%%%%%%%%%%%%%%%%%%%%%%%%%')
print('DONE: final results, step 6.\n Measuring similarity and formatting output was done in %s minutes' % str(round(training_time/60)))
print('%%%%%%%%%%%%%%%%%%%%%%%%%%%\n')